#!/usr/bin/env python3
"""Validate tslearn classification array headers without importing numpy."""
...
class ValidationError(Exception):
    """Raised when an input does not satisfy the tslearn classification contract."""
...
def count_labels(path: Path, has_header: bool) -> int:
    if path.suffix == ".npy":
        ...
        if len(shape) == 2 and 1 in shape:
            return max(shape)
        raise ValidationError(f"{path} labels must have shape (n_ts,), (n_ts, 1), or (1, n_ts)")

    return count_text_labels(path, has_header)


def validate(x_path: Path, y_path: Path, has_header: bool) -> str:
    x_shape = read_npy_shape(x_path)
    if len(x_shape) != 3:
        raise ValidationError(f"X must be 3D (n_ts, sz, d); got shape {x_shape}")
    if any(dim < 0 for dim in x_shape):
        raise ValidationError(f"X has invalid negative dimensions: {x_shape}")

    y_len = count_labels(y_path, has_header)
    if x_shape[0] != y_len:
        raise ValidationError(
            f"label count mismatch: X has {x_shape[0]} series but y has {y_len} labels"
        )

    return f"OK: X shape {x_shape}; y length {y_len}; tslearn expects (n_ts, sz, d)."


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate tslearn classification X.npy and y labels."
    )
    parser.add_argument("x", type=Path, help="Path to X.npy shaped (n_ts, sz, d)")
    ...
    try:
        print(validate(args.x, args.y, args.has_header))
